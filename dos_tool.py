...
def run_dos_triad(structure : structure, basepath="./testruns/", filename="filename", globalparams: dict = {}, localparams:list[dict] = [{},{},{}], cpus=4, templates="./input_templates/DOS/"):
    ...
    run_simulation(program="pw.x", basepath=basepath, filename=f"{filename}_first", params=first_params, template_path=f"{templates}first.in", cpus=cpus)

    # Second nscf calculation
    second_params = globalparams.update(localparams[1])
    run_simulation(program="pw.x", basepath=basepath, filename=f"{filename}_second", params=second_params, template_path=f"{templates}second.in", cpus=cpus)

    # Last DOS calculation
    third_params = globalparams.update(localparams[2])
    run_simulation(program="dos.x", basepath=basepath, filename=f"{filename}_third", params=second_params, template_path=f"{templates}third.in", cpus=cpus)